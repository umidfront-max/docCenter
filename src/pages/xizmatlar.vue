<template>
  <div class="p-4">
    ...
    <el-dialog v-model="dialogVisible" title="Xizmat">
      <el-form>
...
const load = async () => {
  const { data } = await api.get("/xizmatlar/")
  list.value = data
}
...
const submit = async () => {
  if (form.value.id) {
    await api.put(`/xizmatlar/${form.value.id}/`, { name: form.value.name })
  } else {
    await api.post("/xizmatlar/", { name: form.value.name })
  }
